...
Anyone with a GitHub account (even if they have not been added as a collaborator) can create Issues documenting that something "Is not how it should be".
...
- Suggested fix.
    ...
    Additionally, if you are assigning the issues to yourself you don't need to bother being as detail.
- Steps to reproduce
...
Spending the time to creating a well defined issues is mostly about defining a unit of work that I can hand off to someone else.
If you are not planning on handing it off to someone else that does not matter.
...
You are not allowed to main directly, instead you must make a new branch and make your changes there.
...
### Closed
This issues/pull request has been resolved/dealt with. Common reasons for closing something include:
- Fixed
- Duplicate
    Link to what it is a duplicate of. (Five people opening issues about the same typo for example)
- Considered not done
    List reason for why it is not done.
- Stale/Abandoned
    Example If someone started a pull request 2 years ago but it stalled out/ the author lost interest. 
...
### Open 
This issues/pull request is still active.
